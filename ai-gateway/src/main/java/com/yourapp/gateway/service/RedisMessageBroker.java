...
public class RedisMessageBroker {
     ...
     * @param message the message to publish
     */
    public void publishToSession(String sessionId, GatewayMessage message) {
     ...
     * @param handler callback to handle received messages
     */
    public void subscribeToSession(String sessionId, Consumer<GatewayMessage> handler) {
        String channel = "session:" + sessionId;
        subscribe(channel, handler);
    }
    ...
    private void publishMessage(String channel, GatewayMessage message, Long sequenceNumber) {
        try {
            // Add sequence number if provided
            GatewayMessage enriched = message;
            if (sequenceNumber != null) {
                enriched = message.withSequenceNumber(sequenceNumber);
            }

            String json = objectMapper.writeValueAsString(enriched);
            redisTemplate.convertAndSend(channel, json);

            log.debug("Published to Redis channel {}: type={}, seq={}",
                    channel, message.getType(), sequenceNumber);
