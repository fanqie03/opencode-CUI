...
| `service/AsyncTaskService.java` | 任务创建 + 定时扫描调度 |
| `service/task/TaskProcessor.java` | 异步任务处理器接口 |
| `service/task/AsyncTaskContainer.java` | 异步任务容器，收集所有 TaskProcessor 按 AsyncTaskType 路由 |
| `service/listener/SessionDeletedSyncNotifier.java` | 监听 SessionDeletedEvent → 多端同步推送 |

**多端同步基础设施（独立模块 `service/sync/`）**：

| 文件 | 说明 |
|------|------|
| `service/sync/MultiDeviceSyncService.java` | 多端同步接口：`getSyncMode()` + `push(SyncRequest)` |
| `service/sync/CompositeMultiDeviceSyncService.java` | @Primary 路由入口，List<> 注入所有实现，按 SyncMode 分发 |
| `service/sync/WsMultiDeviceSyncService.java` | WS 通道：→ `RedisMessageBroker.publishToUser()`，含 targetAccount null/blank 守卫 |
| `service/sync/ImMultiDeviceSyncService.java` | IM 通道：→ HTTP POST app-notify，类型化响应解析，配置缺失软跳过 |
| `model/SyncMode.java` | WS / IM 枚举 |
| `model/SyncType.java` | SESSION_UNREAD / SESSION_DELETED 枚举 |
| `model/SyncRequest.java` | syncMode + syncType + syncContent + targetAccount record |
| `model/AppNotifyData.java` | IM app-notify data record（type + data） |
| `model/AppNotifyRequest.java` | IM app-notify 请求 body record |
| `model/ImAppNotifyResponse.java` | IM app-notify 响应 record（含 error 嵌套） |
| `config/MultiSyncProperties.java` | `@ConfigurationProperties(prefix = "skill.multi-sync")` |
...
`MultiSyncProperties.java` 通过 `@ConfigurationProperties(prefix = "skill.multi-sync")` 绑定。IM app-notify URL 默认从 `skill.im.api-url` 拼接，运维可 grep 到真实默认值。

##### 6.3.6.2.4 多端同步基础设施

多端同步被抽离为独立基础设施（`service/sync/`），通过 Composite 模式按 `SyncMode` 路由到 WS 或 IM 通道。

**架构**：

```
MultiDeviceSyncService (interface)
├── CompositeMultiDeviceSyncService (@Primary, 路由入口)
│   ├── WsMultiDeviceSyncService → RedisMessageBroker.publishToUser()
│   └── ImMultiDeviceSyncService → HTTP POST /v1/app-notify
└── MultiSyncProperties (@ConfigurationProperties)
```

**Composite 路由**：

```java
@Primary  // 防止 IM/WS/Composite 同时注入时 NoUniqueBeanDefinitionException
@Component
public class CompositeMultiDeviceSyncService implements MultiDeviceSyncService {
    ...
    public CompositeMultiDeviceSyncService(List<MultiDeviceSyncService> services) {
        // List 注入不受 @Primary 影响，三个实现全部进入 registry
    ...
    public void push(SyncRequest request) {
        ...
        if (svc != null) svc.push(request);
    }
}
```

**WS 通道**：`WsMultiDeviceSyncService.push()` 构建 JSON envelope，调用 `RedisMessageBroker.publishToUser()` 推送到 `user-stream:{userId}`。含 targetAccount null/blank 守卫（与 `MiniappDeliveryStrategy` 一致）。

**IM 通道**：`ImMultiDeviceSyncService.push()` 通过 `RestTemplate.postForEntity()` 调用 IM app-notify HTTP API，使用类型化 request/response record（`AppNotifyRequest` / `ImAppNotifyResponse`），`@JsonProperty` 显式锚定 snake_case wire name。URL 未配置时软跳过（WARN 日志，不抛异常）。

**调用方**：

```java
// SessionDeletedSyncNotifier 监听 SessionDeletedEvent → 多端同步
@EventListener
public void notifySessionDeleted(SessionDeletedEvent event) {
    multiDeviceSyncService.push(new SyncRequest(
        SyncMode.WS, SyncType.SESSION_DELETED, content, session.getUserId()));
}
```

后续 PR 将接入未读角标等更多 SyncType 调用方。
