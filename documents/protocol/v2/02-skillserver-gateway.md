# Layer 2：Skill Server ↔ AI Gateway 协议详解（v2）
...
**v2 新增：** Skill Server 通过 `GatewayDiscoveryService` 动态发现所有 Gateway 实例，并与每个实例建立独立 WebSocket 连接。
...
4. 验证 `source` 非空
5. **[v2 新增]** 提取 `instanceId`（若存在），存入 WebSocket 会话属性
6. 将 `source` 存入 WebSocket 会话属性
7. 回显子协议到响应头（RFC 6455 要求）

### 1.2 Gateway 实例自注册（v2 新增）
...
Value:     {"wsUrl":"ws://{host}:8081/ws/skill","startedAt":"..."}
...
**v2 变更：** 从单连接改为按 instanceId 管理多连接。

- 初始化时连接 `seed-gateway-url`（种子地址）
- `GatewayDiscoveryService` 发现新实例 → 自动建立新连接
- 发现实例消失 → 关闭对应连接
- 连接关闭的重连采用 `computeIfPresent` 原子操作避免并发问题：
  ```java
  reconnectFutures.computeIfPresent(instanceId, (id, existing) -> {
      if (!existing.isDone()) return existing;
      return scheduler.schedule(() -> reconnect(instanceId, url), delay, TimeUnit.MILLISECONDS);
  });
  ```
- 重连策略：`delay = min(1000 × 2^(attempt-1), 30000)` ms
- `@PreDestroy destroy()` 优雅关闭所有连接

**SkillRelayService（Gateway 侧）：**

**v2 变更：** 引入 Mesh/Legacy 双策略路由。

- 连接注册时自动判断策略：
  - 握手携带 `instanceId` → **Mesh 策略**（新版 SS）
  - 握手未携带 `instanceId` → **Legacy 策略**（旧版 SS）

#### Mesh 策略（新版）

- 使用本地 `routeCache`（ConcurrentHashMap）缓存路由
- 被动学习：下行 invoke 经过时记录 `source → session` 映射
- **定时清理：** 每 5 分钟扫描 `routeCache`，驱逐指向已关闭连接的条目
  ```
  @Scheduled(fixedDelay = 300_000) // 5 min
  evictStaleRouteCache():
    遍历 routeCache → 检查 session.isOpen()
    已关闭 → 移除条目
    打印清理日志
  ```
...
- Owner 心跳 + Redis relay 跨 Gateway 实例中继
- 连接注册时：
  - 加入连接池
...
**v2 变更：** 根据连接策略分发到 Mesh 或 Legacy 策略。

```
Step 0: [v2] 判断策略
  isLegacyClient(session) → 委托 legacyStrategy.handleInvokeFromSkill()
  否则 → Mesh 策略（本地处理）
...
Step 5: [v2 Mesh] 学习路由
  routeCache.put(source, session)  // 被动学习：记住这个 Source 走哪个连接
...
```
Step 0: 判断策略
  isLegacyClient(defaultLink) → 委托 legacyStrategy.relayToSkill()

Mesh 策略:
  Step 1: 查询 routeCache
    cached = routeCache.get(message.source)
    → 命中且 isOpen() → 直接发送（cache_hit）

  Step 2: 遍历所有连接
    从注册的 Mesh 连接中选择一个活跃连接
    → 发送成功 → 写入 routeCache

  Step 3: 无可用连接 → 日志记录，消息丢失

Legacy 策略:
  Step 1: 尝试本地默认 link
    sendViaDefaultLink(source, message) → 成功 → return

  Step 2: 选择 owner（Rendezvous Hash）
    owners = Redis SMEMBERS gw:source:owners:{source}
    过滤存活实例
    → 本地 owner → 重试本地
    → 远程 owner → Redis PUBLISH gw:relay:{instanceId}
```
...
### 8.2 session_route 表（v2 新增）

```sql
CREATE TABLE session_route (
  id               BIGINT PRIMARY KEY,        -- Snowflake ID
  ak               VARCHAR(64) NOT NULL,      -- Agent AK
  welink_session_id VARCHAR(64),              -- Skill 会话 ID
  tool_session_id  VARCHAR(256),              -- OpenCode 会话 ID
  source_type      VARCHAR(64),               -- 来源类型（如 skill-server）
  source_instance  VARCHAR(128),              -- 来源实例标识
...
**用途：** 持久化会话→Source 实例的路由关系，支持 Gateway 无状态恢复和精准路由。
