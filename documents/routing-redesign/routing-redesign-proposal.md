# Gateway 路由架构重设计方案 v3

> 最终方案：WebSocket 全连接网格 + 被动路由学习 + 广播降级
...
| **Kafka 非关键路径** | 仅用于审计/持久化/回放，不在流式热路径上 |
| **Plugin 零改动** | 协议完全不变 |
...
| 组件 | 当前职责 | 新职责 | 变化 |
|------|---------|--------|------|
| **Gateway** | 连接管理 + 路由决策 + Redis pub/sub 中继 + owner 选举 | 连接管理 + 被动路由缓存 + 直推/广播 | 大幅简化 |
| **Source 服务** | 连单个 GW + 通过 WS 收发 | 连所有 GW + 查 conn:ak 精确下行 + ownership 检查 | 连接管理增强 |
| **Redis** | pub/sub 中继 + 路由表 + owner 心跳 | conn:ak 注册 + GW 实例注册 + 认证缓存 | 大幅简化 |
| **MySQL** | Agent 连接记录 + 会话管理 | + session_route 路由表 | 小增 |
| **Kafka** | 无 | 异步审计/持久化（非关键路径） | 新增（可选） |
...
```java
void onAgentDisconnected(String ak) {
    String current = redis.get("conn:ak:" + ak);
    if (instanceId.equals(current)) {
        redis.delete("conn:ak:" + ak);
    }
    // 若 current != instanceId，说明 Agent 已重连到其他 GW，不删
}
```
...
```java
// GW 侧 SkillRelayService 内部
class RouteCache {
    // toolSessionId → SS 的 WebSocket 连接
    // "w:" + welinkSessionId → SS 的 WebSocket 连接
    private final Cache<String, WebSocketSession> cache = Caffeine.newBuilder()
        .maximumSize(500_000)       // 50 万条（覆盖大部分活跃会话）
...
| 场景 | 处理 |
|------|------|
| SS 连接断开 | `invalidateRoutesForSession(ssSession)` — 遍历缓存移除所有指向该连接的条目 |
| 30 分钟无访问 | Caffeine 自动过期 |
...
50 万条上限完全够用（Caffeine LRU 淘汰低频条目）

每条约 100 bytes（key 64B + value 引用 36B）→ 50 万条 ≈ 50MB 内存
```
...
```
GW 启动时：
  SET gw:instance:{instanceId} '{"wsUrl":"ws://10.0.1.5:8081/ws/skill","startedAt":"2026-03-21T10:00:00Z"}' EX 30

每 10 秒刷新：
  EXPIRE gw:instance:{instanceId} 30
...
```java
// GatewayDiscoveryService（SS 侧）
@Scheduled(fixedDelay = 10000)
public void discoverGateways() {
    // 1. 扫描 Redis 中所有 GW 实例
    Set<String> keys = redis.keys("gw:instance:*");
    Map<String, GwInstanceInfo> discovered = new HashMap<>();
    for (String key : keys) {
        String json = redis.get(key);
        if (json != null) {
            GwInstanceInfo info = parse(json);
            String instanceId = key.replace("gw:instance:", "");
            discovered.put(instanceId, info);
        }
    }

    // 2. 对比当前连接池
    Set<String> currentIds = gatewayWSClient.getConnectedInstanceIds();

    // 3. 新发现的 GW → 建连
    for (String id : discovered.keySet()) {
        if (!currentIds.contains(id)) {
            gatewayWSClient.connectToGateway(id, discovered.get(id).wsUrl());
        }
    }
    ...
    for (String id : currentIds) {
        if (!discovered.containsKey(id)) {
            gatewayWSClient.disconnectFromGateway(id);
        }
    }
}
```
...
废弃 `ak_sk_credential` 表，改为调用外部统一身份校验接口。
...
GW -> RD : Nonce 防重放\nSET NX auth:nonce:{nonce} EX 300
...
alt L1 命中
  ...
  alt L2 命中
    ...
    alt API 成功
      API --> GW : {checkResult:true, userId}
      GW -> CF : 写 L1 (TTL 5min)
      GW -> RD : 写 L2 (TTL 1h)
    else API 失败（降级）
      GW -> DB : SELECT FROM ak_sk_credential\nWHERE ak=? AND status='ACTIVE'
      note right #lightyellow: 降级到本地 DB\n（暂保留不删）
    end
  end
end
...
| 级别 | 条件 | 行为 |
|------|------|------|
| L1 | Caffeine 命中 | 直接使用缓存的 sk 验签 |
| L2 | Redis 命中 | 使用缓存，回填 Caffeine |
| L3 | 外部 API 正常 | 调 API，回填两级缓存 |
| L4 | 外部 API 不可用 | 降级到本地 `ak_sk_credential` 表 |
| L5 | 本地 DB 也查不到 | 拒绝认证 |
...
| `gw:instance:{id}` | `{"wsUrl":"ws://...","startedAt":"..."}` (JSON) | 30s | GW 实例注册 |
| `auth:identity:{ak}` | `{"userId":"...","sk":"..."}` (JSON) | 3600s | 认证缓存 L2 |

**保留的 Key：**

| Key | 用途 | 说明 |
|-----|------|------|
| `agent:{ak}` (pub/sub) | Agent 下行消息投递 | 保留不变（下行仍需跨 GW 实例投递到 Agent） |
...
| Key | 替代方案 |
|-----|---------|
| `gw:agent:source:{ak}` | GW 本地路由缓存（被动学习） |
| `gw:agent:user:{ak}` | 外部 API 认证结果中获取 userId |
| `gw:relay:{instanceId}` | 全连接网格直推，不需要中继 |
| `gw:source:owner:{source}:{instanceId}` | 不需要 owner 选举 |
| `gw:source:owners:{source}` | 不需要 |

### 9.4 关于 agent:{ak} pub/sub 的说明

**为什么保留 `agent:{ak}` pub/sub？**

当前下行路径：SS → WS → GW → Redis PUBLISH `agent:{ak}` → GW subscriber → WS → Agent

新方案下行路径：SS → 查 conn:ak → 选择正确的 GW WS 连接 → GW → WS → Agent

**新方案中 `agent:{ak}` pub/sub 不再需要了**。SS 直接通过 WS 连接发给正确的 GW，GW 本地查 `agentSessions` 找到 Agent 的 WS 连接发送。

因此 `agent:{ak}` pub/sub **也可以废弃**，但建议 Phase 1 先保留作为 fallback，Phase 2 确认稳定后移除。
...
### Phase 1（Gateway 侧，2~3 周）
- 1.1 GatewayInstanceRegistry — GW 实例注册到 Redis
- 1.2 GatewayMessage — 添加 gatewayInstanceId 字段
- 1.3 SkillRelayService **重写** — 多 SS 连接池 + 路由缓存 + 广播降级
- 1.4 SkillWebSocketHandler — 支持 ssInstanceId + 路由学习
- 1.5 EventRelayService **重写** — 上行直推替代 pub/sub
- 1.6 AgentWebSocketHandler — conn:ak 注册
- 1.7 RedisMessageBroker **简化** — 移除中继/owner 选举
- 1.8 AkSkAuthService **重写** — 外部 API + 二级缓存

### Phase 2（Skill Server 侧，1~2 周）
- 2.1 GatewayDiscoveryService — 定期发现 GW 实例
- 2.2 GatewayWSClient **重写** — 全连接池管理
- 2.3 SessionRouteService + Repository — session_route CRUD
- 2.4 GatewayRelayService **重写** — 查 conn:ak 精确投递
- 2.5 GatewayMessageRouter — ownership 检查
...
| 文件 | 改动类型 | TDD 测试类 |
|------|---------|-----------|
| `service/SkillRelayService.java` | **重写** | `SkillRelayServiceTest` |
| `service/EventRelayService.java` | **重写** | `EventRelayServiceTest` |
| `service/RedisMessageBroker.java` | **大幅简化** | `RedisMessageBrokerTest` |
| `ws/SkillWebSocketHandler.java` | 改造 | `SkillWebSocketHandlerTest` |
| `ws/AgentWebSocketHandler.java` | 改造 | `AgentWebSocketHandlerTest` |
| `model/GatewayMessage.java` | 小改 | `GatewayMessageTest` |
| `service/AkSkAuthService.java` | **重写** | `AkSkAuthServiceTest` |
| 新增 `service/GatewayInstanceRegistry.java` | 新增 | `GatewayInstanceRegistryTest` |

### Skill Server 侧 (`skill-server/`)

| 文件 | 改动类型 | TDD 测试类 |
|------|---------|-----------|
| `ws/GatewayWSClient.java` | **重写** | `GatewayWSClientTest` |
| `service/GatewayRelayService.java` | **重写** | `GatewayRelayServiceTest` |
| `service/GatewayMessageRouter.java` | 改造 | `GatewayMessageRouterTest` |
| 新增 `service/SessionRouteService.java` | 新增 | `SessionRouteServiceTest` |
| 新增 `service/GatewayDiscoveryService.java` | 新增 | `GatewayDiscoveryServiceTest` |
| 新增 `repository/SessionRouteRepository.java` | 新增 | — |
| 新增 `model/SessionRoute.java` | 新增 | — |
| 新增 `resources/mapper/SessionRouteMapper.xml` | 新增 | — |
| 新增 `resources/db/migration/V7__session_route.sql` | 新增 | — |
...
### Q2: 连接数 O(N×M) 会不会太多？
20 GW × 20 SS = 400 条 WebSocket 连接。即使扩展到 50×50 = 2500 条，每条 WS 连接开销约 50KB 内存，2500 条 ≈ 125MB 总开销，完全可接受。
...
3. 新服务启动 → 发现所有 GW → 建立 WS 连接（`source_type=bot-platform`）
4. GW 自动将其注册到 `sourceTypeSessions["bot-platform"]` 池
5. 路由、广播、缓存全部自动生效
